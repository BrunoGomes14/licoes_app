...
public class AdapterLicao extends RecyclerView.Adapter<AdapterLicao.HolderLicao>
{
    Context context;
    ...
    public AdapterLicao(Context context, ArrayList<modelImagem> imagems, int iTipo, boolean bEntregou)
    {
        this.context = context;

        this.imagens.clear();
        for (modelImagem img: imagems) {
            if (img.isDuvida == 0)
            {
                imagens.add(img);
            }
    ...
    @NonNull
    @Override
    public HolderLicao onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_imagem, parent, false);
        HolderLicao holder = new HolderLicao(view);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull HolderLicao holder, int position) {
...
//        }
    ...
    class HolderLicao extends RecyclerView.ViewHolder {

        public RelativeLayout rnlPrincipal;
        public ImageView img;
        public ImageView imgFechar;

        public HolderLicao(@NonNull View itemView) {
            super(itemView);
